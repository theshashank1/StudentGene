...
const Quiz = () => {
  ...
  useEffect(() => {
    const dataFetching = async () => {
      try {
        const response = await axios.get("https://669234d4346eeafcf46c2910.mockapi.io/studentgenie/v1/api/quiz");
  ...
  useEffect(() => {
    console.log("Current index:", index); // Log the index whenever it changes
  }, [index]);
  ...
  const checkCorrect = (idx) => {
    ...
    if (isCorrect) {
      setScore(prevScore => prevScore + 1); // Update score if the answer is correct
    }
